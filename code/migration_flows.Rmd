...
s19_s23 %>% 
  ...
  group_by(State) %>% 
   do(model = lm(value ~ name, data = .)) %>%   # Run linear model for each state
  summarise(
    state = State,
    p_value = summary(model)$coefficients[2, 4],  # Extract p-value for the year coefficient
    estimate = summary(model)$coefficients[2, 1]  # Extract estimate (slope)
  )
...
for (i in 2:nrow(net_migration)) {  # Start from the second row (2022)
  ...
  net_migration$percentage_change[i] <- ((net_migration$net_perc[i] - net_migration$net_perc[i-1]) / net_migration$net_perc[i-1]) * 100
}
