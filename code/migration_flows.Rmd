...
graph1 <- net_migration %>% 
  ...
  labs(y = "Population (per 1000)",
       x = "Year",
       color = "",
       title = "Migration In and Out of New York State",
       caption = str_wrap("Graph 1: Time trends for migration of people in and out of NYS between 2015 and 2023. The spike between 2019 and 2021 follows the trends seen during the COVID pandemic.")) +
  scale_x_continuous(
...
ggsave(here("outputs", "timeTrend_migration.png"), plot = graph1, width = 10, height = 6, dpi = 300)
...
graph2 <- net_migration %>% 
  ...
  geom_col(fill = "#606C38") +
   labs(y = "Population (per 1000)",
       x = "Year",
       color = "",
       title = "Net Migration to and from New York State",
       caption = str_wrap("Graph 2: Time trends for net migration of people in and out of NYS between 2015 and 2023. Negative values in each year indicate that every year from 2015-2023, more people are migrating out of NYS than migrating in.")) +
   scale_x_continuous(
    breaks = unique(net_migration$year),   # Explicitly set breaks for each year
    labels = as.character(unique(net_migration$year))  # Show years as labels
  ) +
...
ggsave(here("outputs", "timeTrend_net.png"), plot = graph2, width = 10, height = 6, dpi = 300)
```
...
in_table <- gt(in_migration) %>% 
  ...
  tab_source_note(
    source_note = md("**Table 2: Top 5 states and foreign migration into New York State from 2019-2023.** For each year, the top source was from foreign countries. The top 5 states migrating to NYS were New Jersey, California, Pennsylvania, Florida, Massachusetts.")
  )
...
state_in <- s19_s23 %>% 
  ...
  labs(title = "Top Sources of Migration Into New York State", 
       x = "Year", 
       y = "Persons (per 1000 people)",
       fill = "Source",
       caption = str_wrap("Graph 3: Top sources of migration into NYS from 2019-2023.")) +
...
state_out <- ss19_23 %>% 
  ...
  ggplot(aes(x = Year, y = `Migration From NYS`/1000, fill = reorder(Destination, `Migration From NYS`)))+
  geom_col(position = "dodge") +
  geom_text(aes(label = paste0(round(`% From NYS`, 0), "%")),  # Add labels above the bars
            position = position_dodge(width = 0.8), vjust = -0.5, size = 3) +
  labs(title = "Top Destinations of Out-Migration From New York State", 
       x = "Year", 
       y = "Persons (per 1000 people)",
       fill = "",
       caption = str_wrap("Graph 3: Top Destinations from NYS 2019-2023. The percentages show the contribution of total migration to that state that are from New York State. NYS consistently makes up the most of the migration to Connecticut and New Jersey.")) +
